...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  //if (newNode->data == newData){
  //std::cout<<"newNode->data is "<<newNode->data<<"\n";
  //}
  //if (newData <= )
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  //if both left and right empty, return an empty list
  if (left.empty() && right.empty()){
    return merged;
  }
  //while one of the lists is not empty  
  while(!left.empty() || !right.empty()){
    //if left empty and right not empty, add right to merged, pop right
    if (left.empty() && !right.empty()){
      merged.pushBack(right.head_->data);
      right.popFront();
    }
    //if left not empty and right empty, add left to merged, pop left
    else if (!left.empty() && right.empty()){
      merged.pushBack(left.head_->data);
      left.popFront();
    }
    //if right data < left data, add right to merged, pop right
    else if (right.head_->data <= left.head_->data){
      merged.pushBack(right.head_->data);
      right.popFront();
    }
    //if left data < right data, add left to merged, pop left
  ...
  //if right empty and left full, remove from left, add to merged
  //pick left if left<= right, else pick right
